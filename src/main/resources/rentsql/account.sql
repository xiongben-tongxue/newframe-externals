...
(
   ...
   useable_amount       decimal(10,2) not null comment '可用余额',
   frozen_amount        decimal(10,2) not null comment '冻结金额',
   due_amount           decimal(10,2) not null comment '待还金额',
   account_status       tinyint(1) NOT NULL COMMENT '账号状态,1:正常,2:冻结',
...
(
   ...
   cash_deposit         decimal(10,2) not null comment '保证金',
   due_amount           decimal(10,2) not null comment '待收金额',
   account_status       tinyint(1) NOT NULL COMMENT '账号状态,1:正常,2:冻结',
   ctime                bigint not null comment 'ctime',
   utime                bigint comment 'utime',
   primary key (uid)
...
(
   ...
   invest_amount        decimal(10,2) comment '投资金额(融资购机没有此项)',
   invest_deadline      decimal(10,2) not null comment '投资期限',
   repay_way            int not null comment '还款方式（1：押金贷，2：按月支付）',
   due_amount           decimal(10,2) not null comment '待还金额',
   repay_amount         decimal(10,2) not null comment '已还金额',
...
(
   ...
   rent_deadline        decimal(10,2) not null comment '租机期限',
   matter_price         decimal(10,2) not null comment '租赁物购买价款',
   renter_id            bigint comment '承租人ID',
   renter_name          varchar(10) comment '承租人店铺名称',
   final_renter         bigint comment '终端承租人',
   final_renter_name    varchar(10) comment '终端承租人姓名',
   ctime                bigint not null comment 'ctime',
   utime                bigint comment 'utime',
   primary key (id)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(
   ...
   useable_amount       decimal(10,2) not null default 0 comment '可用余额',
   total_assets         decimal(10,2) not null default 0 comment '总资产',
   frozen_assets        decimal(10,2) not null default 0 comment '总资产',
   margin_balance       decimal(10,2) not null default 0 comment '保证金余额',
   margin_advances      decimal(10,2) not null default 0 comment '保证金垫付金额',
   due_amount           decimal(10,2) not null default 0 comment '待收金额',
   account_status       tinyint(1) NOT NULL COMMENT '账号状态,1:正常,2:冻结',
   ctime                bigint not null comment 'ctime',
   utime                bigint comment 'utime',
   primary key (uid)
...
(
   ...
   financing_principle  decimal(10,2) not null comment '融资本金',
   total_interest       decimal(10,2) not null comment '融资总利息',
   repay_principal      decimal(10,2) not null default 0 comment '已清偿本金',
   repay_interest       decimal(10,2) not null default 0 comment '已清偿利息',
...
(
   ...
   order_id             bigint not null comment '订单id',
   invest_way           int not null comment '投资方式（1：融资购机，2：租机）',
   invest_amount        decimal(10,2) comment '投资金额(押金贷没有此项)',
   invest_deadline      decimal(10,2) not null comment '投资期限',
   margin_advances      decimal(10,2) not null comment '保证金垫付金额',
   repay_way            int not null comment '还款方式（1：押金贷，2：按月支付）',
...
(
   ...
   rent_amount          decimal(10,2) not null comment '租机总金额',
   rent_deadline        int not null comment '租机期限',
   repay_amount         decimal(10,2) not null comment '已付租金',
...
(
   uid                  bigint not null comment 'uid',
   useable_amount       decimal(10,2) not null comment '可用金额',
   total_asset          decimal(10,2) not null comment '资产总额',
   frozen_asset         decimal(10,2) not null comment '冻结资产',
   ctime                bigint not null comment 'ctime',
   utime                bigint comment 'utime',
   primary key (uid)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(
   ...
   price                decimal(10,2) not null comment '购买价款',
...
(
   ...
   repay_amount         decimal(2,2) not null comment '应还总金额',
...
(
   ...
   repay_amount         decimal(10,2) not null comment '应还总金额',
   repay_principle      decimal(10,2) not null comment '应还本金',
   repay_interest       decimal(10,2) comment '应还利息(押金贷有此项)',
   last_repay_time      bigint not null comment '最迟还款日期',
   repay_status         int not null comment '还款状态(1：未扣款，2：已扣款，3：已逾期，4：逾期已结清)',
   ctime                bigint not null comment 'ctime',
   utime                bigint comment 'utime',
