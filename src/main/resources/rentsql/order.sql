...
CREATE TABLE `order_renter` (
  ...
  `user_realname` varchar(64) NOT NULL COMMENT '用户真实姓名',
  `user_id_number` varchar(64) NOT NULL COMMENT '用户身份证号码',
  `user_mobile` varchar(64) NOT NULL COMMENT '用户手机号',
  `user_credit_score` INTEGER NOT NULL COMMENT '用户信用分',
  `user_credit_line` tinyint(1) NOT NULL COMMENT '用户的信用度评级，1:信用极好,2:信用中等,3:信用较差',
  `product_brand` varchar(64) NOT NULL COMMENT '产品品牌',
  `product_name` varchar(64) NOT NULL COMMENT '产品名字',
  `product_color` varchar(64) NOT NULL COMMENT '产品颜色',
  `product_storage` integer NOT NULL COMMENT '产品内存GB',
  `monthly_payment` decimal(10,2) NOT NULL COMMENT '月租金金额(月还款数)',
  `number_of_payments` integer NOT NULL COMMENT '还贷月数(租期)',
...
CREATE TABLE `financing_buy_machine` (
  ...
  `merchant_name` varchar(64) NOT NULL COMMENT '租赁商家名字',
  `funder_id` bigint NOT NULL COMMENT '出租方ID',
  `funder_name` varchar(64) NOT NULL COMMENT '出租方名字',
  `uid` bigint NOT NULL COMMENT '用户ID',
  `user_realname` varchar(64) DEFAULT NULL COMMENT '用户真实姓名',
  `user_id_number` varchar(64) DEFAULT NULL COMMENT '用户身份证号码',
  ...
  `dispatch_times` varchar(50) NOT NULL COMMENT '派单次数',
...
CREATE TABLE `rent_machine` (
  ...
  `hirer_name` varchar(64) NOT NULL COMMENT '出租方名字',
  `uid` bigint NOT NULL COMMENT '用户ID',
  `user_realname` varchar(64) DEFAULT NULL COMMENT '用户真实姓名',
  `user_id_number` varchar(64) DEFAULT NULL COMMENT '用户身份证号码',
  `user_credit` tinyint(1) NOT NULL COMMENT '用户的信用度评级，1:信用极好,2:信用中等,3:信用较差',
...
CREATE TABLE `order_supplier` (
  ...
  `receiver_mobile` varchar(30) DEFAULT NULL COMMENT '用户手机号',
  ...
  `order_status` varchar(512) DEFAULT NULL COMMENT '订单状态,1:待处理,2:已发货,3:待收货,4:已收货',
